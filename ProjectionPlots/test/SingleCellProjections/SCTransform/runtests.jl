...
@testset "SCTransform.jl" begin
	@testset "500_PBMC_50genes" begin
		...
		filename = joinpath(@__DIR__, "../SingleCell10x/data/500_PBMC_3p_LT_Chromium_X_50genes/filtered_feature_bc_matrix.h5")
		X,f,b = read10x(filename)

		base = joinpath(@__DIR__, "data/500_PBMC_50genes")

		pnonreg_ans = read_model_pars(joinpath(base, "model_pars.csv"))
		gm_ans = readdlm(joinpath(base, "genemean.csv"), Float64)

		pnonreg_ans2 = read_model_pars(joinpath(base, "model_pars2.csv"))
		bw_ans2 = 0.243867597143564
		preg_ans2 = read_model_pars(joinpath(base, "model_pars_fit2.csv"))
		gm_ans2 = readdlm(joinpath(base, "genemean2.csv"), Float64)
		Z_ans2 = readdlm(joinpath(base, "transformed2_every_10th_cell.csv"), ',', Float64)
