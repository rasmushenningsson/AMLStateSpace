...
# TODO: project for PMAModel


struct NearestNeighborModel <: ProjectionModel
	name::String
	pre::LowRankFactorization
	post::Matrix{Float64}
	var_match::DataFrame
	k::Int
	var::String
...
NearestNeighborModel(name, pre::DataMatrix, post; k, var, obs) =
	NearestNeighborModel(name, pre.matrix, post, select(pre.var,pre.var_id_cols), k, var, obs)
...
function project_impl(data::DataMatrix, model::NearestNeighborModel; adj_out=nothing, verbose=true)
	...
	adj, matrix = embed_points(obs_coordinates(model.pre), model.post, obs_coordinates(data); model.k)
