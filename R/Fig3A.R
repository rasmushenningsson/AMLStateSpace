...
new.anno <- FetchData(AML.merged, vars = c("orig.ident", "seurat_clusters", "celltype.l4")) %>%   rownames_to_column() %>% 
  mutate(celltype.aml = case_when(seurat_clusters %in% c(AMLIm) & !grepl("NBM",orig.ident) ~ "AML Immature", 
                                  TRUE ~ celltype.l4)) %>% 
  column_to_rownames()
